...
This document describes a clean-architecture reinforcement learning environment, `HoldingAgentLidarEnv`, for training an aircraft to maintain a holding pattern inside a circular containment zone while avoiding circular No-Fly Zones (NFZs). The design intentionally separates simulator I/O from environment logic: the environment is Gymnasium-compliant and focuses on observations, reward shaping, reset randomization, and step orchestration, while simulator-specific implementations (e.g., BlueSky now, JSBSim later) are hidden behind a `SimulatorInterface` contract. This makes the core task logic testable via mocks, portable across simulator backends, and reusable for future environments.

Algorithmically, the agent learns a continuous-control policy to command heading-rate changes based on a compact state representation: normalized distance and bearing-to-center plus a 360° LIDAR-like ring of ray-cast distances to nearby NFZs. LIDAR is simulated efficiently using ray-circle intersections (keeping the nearest hit per ray), while coordinate transforms map simulator lat/lon into a local XY plane (accurate azimuthal-equidistant via `pyproj`, with a fast equirectangular fallback). The reward function combines a small survival incentive with strong penalties for boundary violations and NFZ incursions, plus a smooth proximity penalty to supply learning gradients; violations do not terminate episodes to encourage recoverable behavior under PPO-style on-policy training.
...
flowchart TB
  %% Training stack
  subgraph T[Training]
    A[Trainer Script / Experiment Runner]
    PPO[PPO - on-policy RL]
    PI[Policy / Value Networks]
    A --> PPO --> PI
  end

  %% RL interface
  PI -->|action: Box 1<br/>Continuous -1 to 1<br/>scaled to heading_change_deg| ENV
  ENV -->|observation: Box 2 + num_rays<br/>dist_to_center_norm, rel_bearing_norm, lidar_rays_norm| PI

  %% Environment internals
  subgraph ENV[HoldingAgentLidarEnv - Gymnasium Env]
    OBS[Observation builder
- distance to center - normalized
- relative bearing - normalized
- LIDAR rays - normalized]
    LIDAR[LIDAR simulation
ray-circle intersections
closest hit per ray]
    REW[Reward / constraints
- survival reward
- boundary penalty
...
lat/lon -> local XY
pyproj aeqd or equirectangular]
    STEP[Step loop
send heading change
advance sim - dt
query state]
  ...
  %% Simulator boundary
  ENV -->|SimulatorInterface
reset/step/get_state/set_heading_change| SIM

  subgraph SIM[Simulators]
    B[BlueSky simulator adapter]
    J[JSBSim simulator adapter - future]
  end

  %% Dependencies
  subgraph D[Key Python Dependencies]
    G[gymnasium]
    N[numpy]
    P[pyproj - optional]
  end
